...
describe('Method: verifyPresentationResponse()', () => {
  it('should error on missing `vp_token`', async () => {
    const rejected = await assertRejects(() =>
      ...
      verifyPresentationResponse({ data: {}, nonce: '', expectedOrigin: '' })
    );
  ...
  it('should error on bad `vp_token`', async () => {
    const rejected = await assertRejects(() =>
      ...
      verifyPresentationResponse({ data: { vp_token: '' }, nonce: '', expectedOrigin: '' })
    );
  ...
  // TODO: Revisit this test once we figure out stateless stuff
